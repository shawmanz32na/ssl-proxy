...
printf "\n\n ********** STARTING HTTPD HTTPS/AUTH PROXY ********** \n\n\n"
...
DHPARAM_PATH=${DHPARAM_PATH-"/usr/local/apache2/conf/extra/dhparams.pem"}
HTTPS_PORT=${HTTPS_PORT-"443"}
# TODO: Customizable HTTPS settings
...
# Create a tweaked version of the default httpd.conf
# Taken from HTTPD 2.4.43 as provided by httpd:2.4.43-alpine
...
# ssl-proxy customization - enable the modules required for proxying requests
...
#
# ssl-proxy customization - don't include a ServerAdmin
...
# ssl-proxy customization - set the server name
...
#
# ssl-proxy customization - Disable the default DocumentRoot, since the point is to proxy requests
...
# ssl-proxy settings
Include conf/extra/ssl-proxy-ssl.conf
Include conf/extra/ssl-proxy-proxy.conf

EOF

echo "Creating ssl-proxy-ssl.conf..."

# Per Mozilla, add their dhparams to the certificate...
# curl https://ssl-config.mozilla.org/ffdhe2048.txt >> $CERT_PUBLIC_PATH

cat << EOF > /tmp/ssl-proxy-ssl.conf
# generated 2020-04-01, Mozilla Guideline v5.4, Apache 2.4.43, OpenSSL 1.1.1d, intermediate configuration
...
# ssl-proxy customization - load the required modules
<IfModule !mod_ssl>
  LoadModule ssl_module modules/mod_ssl.so
</IfModule>
...
# ssl-proxy customization - actually listen on the HTTPS port
Listen $HTTPS_PORT
...
<VirtualHost *:$HTTPS_PORT>
    ...
    SSLCertificateFile      $CERT_PUBLIC_PATH
    SSLCertificateKeyFile   $CERT_PRIVATE_PATH
    # curl https://ssl-config.mozilla.org/ffdhe2048.txt >> /certs/dhparam.pem
    # ssl-proxy customization - specify the DH Params rather than append them to the certificate like Mozilla suggests
...
# ssl-proxy customization - use the SSL Session Cache at the location provided by the container
SSLSessionCache        "shmcb:/usr/local/apache2/logs/ssl_scache(512000)"
SSLSessionCacheTimeout  300

SSLUseStapling On
# ssl-proxy customization - use the SSL Stapling Cache location provided by the container
...
echo "Creating ssl-proxy-proxy.conf..."

cat << EOF > /tmp/ssl-proxy-proxy.conf
# Default container config
ProxyRequests Off  <-- this is an important security setting
ProxyPass / http://$UPSTREAM_TARGET/
RequestHeader add X-Forwarded-Proto https
RequestHeader add X-Forwarded-Port 443
# TODO: Add any configured header(s) here
<Location />
  ProxyPassReverse /
  ProxyHTMLEnable On
  ProxyHTMLURLMap http://$UPSTREAM_TARGET/ /
  ProxyHTMLURLMap / /
</Location>
...
# ProxyPass / http://$UPSTREAM_TARGET/
# ProxyPassReverse / http://$UPSTREAM_TARGET/
# RequestHeader add X-Forwarded-Proto https
# RequestHeader add X-Forwarded-Port 443
# RequestHeader set HTTP_CLIENT_CERT "%{SSL_CLIENT_S_DN}s"

EOF

echo "Moving HTTPD configuration into place"

cp /tmp/httpd.conf /usr/local/apache2/conf/
cp /tmp/ssl-proxy-ssl.conf /usr/local/apache2/conf/extra/
cp /tmp/ssl-proxy-proxy.conf /usr/local/apache2/conf/extra/
